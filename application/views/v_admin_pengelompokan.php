					...
					<div class="box-body">
					  <table width="100%">
							...
							<div class="form-group">
								<select type="text" class="form-control select2" onchange="if (this.value) window.location='<?php echo base_url();?>admin/pengelompokan/<?php echo $geturlkelas;?>/'+this.value+'/<?php echo $geturljumlahgroup;?>'">
									<?php
									if($geturltopik!="0"){
							...
							<td width="25%">
							<div class="form-group">
								<select type="text" class="form-control select2" onchange="if (this.value) window.location='<?php echo base_url();?>admin/pengelompokan/'+this.value+'/<?php echo $geturltopik;?>/<?php echo $geturljumlahgroup;?>'">
									<?php
									$geturlkelas = $this->uri->segment(3);
									if($geturlkelas!="0"){
							...
							<td width="25%">
							<div class="form-group">
								<select type="text" class="form-control select2" onchange="if (this.value) window.location='<?php echo base_url();?>admin/pengelompokan/<?php echo $geturlkelas;?>/<?php echo $geturltopik;?>/'+this.value">
									<?php
									if($geturljumlahgroup!="0"){
										echo "<option value='".$geturljumlahgroup."'>Jumlah Kelompok ".$geturljumlahgroup."</option>";
									}else{
										echo "<option>-- Pilih Jumlah Kelompok --</option>";
									}
									
									for($group = 1; $group <=$jumlah_mahasiswa_kelas; $group++){
										echo "<option value='".$group."'>Jumlah Kelompok ".$group."</option>";
									}
									?>
								</select>
							</div>
							...
							<td width="25%" style="text-align:right;">
								<?php $attribut = array('target' => '', 'id' => 'aksi');
									echo form_open_multipart("admin/pengelompokan_aksi", $attribut);	
									echo "<input type='hidden' name='jumlah_group' value='$geturljumlahgroup'>
									<input type='hidden' name='kelas_id' value='$geturlkelas'>
									<input type='hidden' name='topik_group' value='$geturltopik'>";
									?>
								<div class="form-group">
									<input type="submit" onclick="return confirm('Yakin untuk submit?');" form="aksi" class="btn btn-success" value="Tentukan">
								</div>
								<?php echo form_close(); ?>
							</td>
							</tr>
						...
						for($transformer=0; $transformer<$tampil_baris; $transformer++){
							echo "<tr>";
								for($urutan = $jumlah_kelompok*$transformer+1; $urutan <= $jumlah_kelompok*($transformer+1) ; $urutan++){
									...
									$series = $urutan-($jumlah_kelompok*$transformer);
									
									echo "<td>"; 
									echo "<img src='".base_url()."assets/avatar/".$data_sorting_user_image[$urutan]."?".strtotime("now")."' width='60px' height='60px'>";
									echo "<br>";
									echo "Nama: ".$data_sorting_user_fullname[$urutan];
									echo "<br>";
									echo "NIM: ".$data_sorting_user_nim[$urutan]; 
									echo "<br>";
									//echo "Kelompok: ".$series; 
									echo "
									<input type='hidden' name='user_nim[$urutan]' value='$data_sorting_user_nim[$urutan]'>
									<input type='hidden' name='nomor_kelompok[$urutan]' value='$series'>
									<input type='hidden' name='kelas_id[$urutan]' value='$geturlkelas'>
									<input type='hidden' name='topik[$urutan]' value='$geturltopik'>";
								}
							echo "</tr>";
						}
}						
